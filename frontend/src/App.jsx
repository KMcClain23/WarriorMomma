...
function getSpice(book) {
  const raw =
    book?.spice ?? book?.spice_level ?? book?.spiceLevel ?? book?.spiceRating ??
    book?.spice_rating ?? book?.['spice level'] ?? book?.['Spice Level'] ??
    book?.['Spice'] ?? book?.['spice'];
...
function getCover(b){const u=b?.coverUrl||b?.cover_image_url||'';return u?u.replace(/^http:/,'https:').replace(/&edge=curl/g,''):'';}
function getGenres(b){if(Array.isArray(b?.genres))return b.genres.filter(Boolean);const r=b?.genre||b?.['genre/theme']||b?.['genre/category']||'';return String(r).split(',').map(s=>s.trim()).filter(Boolean);}
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      try{
        ...
        if(!cancel) setCache(p=>({...p,[sectionKey]:Array.isArray(data)?data:[]}));
      } finally {
  ...
  const availableGenres=useMemo(()=>{
    const s=new Set(); (cache[sectionKey]||[]).forEach(b=>getGenres(b).forEach(g=>s.add(g))); return s;
  },[sectionKey,cache]);
  ...
  const handleOpenModal=(b=null)=>{setEditingBook(b);setIsModalOpen(true)};
  ...
  const handleSaveBook=async (bookData)=>{
    ...
    setCache(prev=>({...prev,[sectionKey]:editingBook?prev[sectionKey].map(b=>b.id===editingBook.id?saved:b):[...(prev[sectionKey]||[]),saved]}));
    handleCloseModal();
  };

  const handleDeleteBook=async (id)=>{
    await fetch(`${endpoint}/${id}`,{method:'DELETE'});
    setCache(prev=>({...prev,[sectionKey]:prev[sectionKey].filter(b=>b.id!==id)}));
  };

  const handleMoveBook=async (book,dst)=>{
    const src=sectionKey;
    const bookToMove=cache[src].find(b=>b.id===book.id);
    setCache(prev=>({...prev,[src]:prev[src].filter(b=>b.id!==book.id),[dst]:[...(prev[dst]||[]),bookToMove]}));
    await fetch(`/api/move-book`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({bookId:book.id,sourceSection:src,destinationSection:dst})});
  };

  const handleUpdateBookStatus=async (book,field,value)=>{
    const updated={...book,[field]:value};
    setCache(prev=>({...prev,[sectionKey]:prev[sectionKey].map(b=>b.id===book.id?updated:b)}));
    await fetch(`${endpoint}/${book.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({[field]:value})});
  };
  ...
  const filtered=useMemo(()=>data.filter(b=>{
    ...
    const genresOk=selectedGenres.size===0||[...selectedGenres].every(g=>genres.includes(g));
    const spiceOk=spiceFilter==='any'?true:spice===Number(spiceFilter);
    const statusOk=statusFilter==='any'?true:statusFilter==='read'?!!b.isRead:!!b.isTbr;
      ...
      <header className="relative">
        ...
        <div className="mt-4 sm:mt-8 border-b border-white/10 -mx-4 sm:mx-0 px-4 tabbar-scroll">
          <div className="flex gap-2 min-w-max">
            {tabs.map(t=>(
              <button key={t.key}
                className={`px-4 py-2 text-base sm:text-lg rounded-t-[14px] hover:text-white transition-colors ${sectionKey===t.key?'text-white border-b-2 border-[var(--gold-ritual)]':'text-white/70'}`}
                onClick={()=>setActive(t)}>{t.label}</button>
            ))}
          </div>
        </div>
      </header>
        ...
        {loading?(
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {filtered.map(b=>{
              ...
              const spice=getSpice(b);
              return (
                <article key={b.id} className={`card hover:shadow-glow transition relative ${moveMenuOpenFor===b.id?'z-20':'z-auto'}`}>
                  <div className="absolute top-2 right-2 flex gap-2 z-10">
                    <button onClick={()=>handleUpdateBookStatus(b,'isRead',!b.isRead)}
                      className={`btn text-xs px-3 py-2 ${b.isRead?'bg-[var(--gold-ritual)] text-[var(--raven-ink)] border-transparent':'btn-phantom'}`}>
                      Read
                    </button>
                    <button onClick={()=>handleUpdateBookStatus(b,'isTbr',!b.isTbr)}
                      className={`btn text-xs px-3 py-2 ${b.isTbr?'bg-[var(--violet-phantom)] text-white border-transparent':'btn-phantom'}`}>
                      TBR
                    </button>
                  </div>

                  {cover?(
                    <img src={cover} alt={b.title} className="w-full h-auto rounded-t-[14px] aspect-[2/3] object-cover"
                      onError={e=>{ if(e.currentTarget.dataset.fallbackApplied) return; e.currentTarget.dataset.fallbackApplied='1'; e.currentTarget.src=fallback; }}
                      loading="lazy" />
                  ):(
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-4 flex gap-2 items-center">
                      <button onClick={()=>handleOpenModal(b)} className="btn bg-[var(--violet-phantom)] text-white hover:opacity-90">Edit</button>
                        ...
                        {moveMenuOpenFor===b.id&&(
                          <div className="absolute top-full right-0 mt-2 w-48 bg-[var(--raven-ink)] rounded-[14px] shadow-lg z-10 ring-1 ring-white/10">
                            {tabs.filter(t=>t.key!==sectionKey).map(t=>(
                              <button key={t.key} onClick={()=>{handleMoveBook(b,t.key); setMoveMenuOpenFor(null);}}
